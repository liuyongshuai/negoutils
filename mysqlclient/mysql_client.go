...
//提取查询SQL的结果
func reFormatRowsData(rows *sql.Rows) (ret []map[string]convertutils.ElemType, err error) {
	if rows == nil {
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		tmp := make(map[string]convertutils.ElemType)
		//根据不同的类型来将[]byte转换一下
		for i, colByteVal := range vals {
			colType := colTypes[i]
			realVal := convertMySQLType(colByteVal, colType)
			tmp[cols[i]] = convertutils.MakeElemType(realVal)
		}
		ret = append(ret, tmp)
	}
	return ret, nil
...
//提取单行的单个字段
func (my *DBase) FetchOne(sql string, args ...interface{}) (ret convertutils.ElemType, err error) {
...
//提取所有行的第一个字段的列表
func (my *DBase) FetchCols(sql string, args ...interface{}) ([]convertutils.ElemType, error) {
...
//提取一行数据
func (my *DBase) FetchRow(sql string, args ...interface{}) (map[string]convertutils.ElemType, error) {
...
//提取多行数据
func (my *DBase) FetchRows(sql string, args ...interface{}) ([]map[string]convertutils.ElemType, error) {
	if my.Db == nil {
...
func (my *DBase) FetchCondRows(table string, cond map[string]interface{}, fields ...string) (ret []map[string]convertutils.ElemType, err error) {
...
func (my *DBase) FetchForUpdate(table string, cond map[string]interface{}) (map[string]convertutils.ElemType, error) {
	cd, param := FormatCond(cond, "AND")
