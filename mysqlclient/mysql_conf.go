...
func FormatCond(cond map[string]interface{}, delim string) (sqlCond string, param []interface{}) {
	...
	var tmpCond []string
	var condItem = make(map[string]convertutils.ElemType)
	for k, v := range cond {
		condItem[k] = convertutils.MakeElemType(v)
	}
	...
	for k, v := range condItem {
		...
		if tmpSym == "in" || tmpSym == "notin" {
			var vslice []convertutils.ElemType
			if v.IsString() && vlen > 0 { //如果是字符串则用"，"切成slice
				...
				for _, t := range tmp {
					...
					vslice = append(vslice, convertutils.MakeElemType(t))
				}
...
//转换查询SQL用的参数
func ConvertArgs(param []convertutils.ElemType) []interface{} {
