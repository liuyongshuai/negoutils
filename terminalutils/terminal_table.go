...
// @file        terminal_table.go
// @date        2020-01-25 18:16
...
type TerminalTable struct {
	//表格的原始数据，目前只有表头和行内容
	rawHeaderData []rawTerminalTableCell   //原始的表头数据
	rawRowData    [][]rawTerminalTableCell //原始的行的数据
...
func (t *TerminalTable) SetHeader(header []string) *TerminalTable {
	...
	for _, h := range header {
		t.rawHeaderData = append(t.rawHeaderData, rawTerminalTableCell{content: h})
	}
	return t
}
...
//添加一行数据
func (t *TerminalTable) AddRow(row []string, colors ...ColorType) *TerminalTable {
	var tmp []rawTerminalTableCell
	for i, v := range row {
		var color ColorType
		if len(colors) > i {
			color = colors[i]
		}
		tmp = append(tmp, rawTerminalTableCell{content: v, color: color})
	}
	t.rawRowData = append(t.rawRowData, tmp)
...
//原始的小格子的内容信息
type rawTerminalTableCell struct {
	content string    //单元格的内容
	color   ColorType //颜色信息
}
	...
	if headerLen > 0 {
		if headerLen < t.maxColumnNum {
			for i := headerLen; i < t.maxColumnNum; i++ {
				t.rawHeaderData = append(t.rawHeaderData, rawTerminalTableCell{content: " "})
			}
		}
	}
	for idx, row := range t.rawRowData {
		if len(row) < t.maxColumnNum {
			for i := len(row); i < t.maxColumnNum; i++ {
				t.rawRowData[idx] = append(t.rawRowData[idx], rawTerminalTableCell{content: " "})
			}
		}
	}
...
func (t *TerminalTable) getMaxColumnWidths() {
	...
	for idx, row := range t.rawHeaderData {
		t.maxColumnWidth[idx] = RuneStringWidth(row.content)
	}
	for _, rows := range t.rawRowData {
		for idx, row := range rows {
			l := RuneStringWidth(row.content)
...
//表头字体获取
func (t *TerminalTable) headerStr(str string, color ColorType) string {
	if t.headerFontColorFunc != nil {
		return t.headerFontColorFunc(str)
	}
	if cfun, ok := ColorFuncMap[color]; ok {
		return cfun(str)
	}
	return Yellow(str)
}
...
//行内容字体获取
func (t *TerminalTable) rowStr(str string, color ColorType) string {
...
func (t *TerminalTable) renderSingleRow(row *terminalTableRow) string {
	...
	for i := 0; i < srowNum; i++ {
		for j := 0; j < colNum; j++ {
			...
			cellObj := row.cellList[j]
			str := cellObj.cellStrList[i]
			switch row.rowType {
			case rowTypeHeader:
				str = t.headerStr(str, cellObj.color)
			case rowTypeData:
				str = t.rowStr(str, cellObj.color)
			}
			buf.WriteString(str)
		}
		//最后一个分隔符
...
func (t *TerminalTable) wrapTableRows(rawRow []rawTerminalTableCell) (retRow *terminalTableRow) {
	if len(rawRow) <= 0 {
		return nil
	}
	...
	for idx, row := range rawRow {
		rawRow[idx].content = replaceList.Replace(row.content)
	...
	for idx, cellStr := range rawRow {
		...
		lineNum := 1
		if RuneStringWidth(cellStr.content) > cellWidth {
			cellStr.content, lineNum = RuneWrap(cellStr.content, cellWidth)
		}
		if lineNum > maxLineNum {
			maxLineNum = lineNum
		}
		tmp := splitReg.Split(cellStr.content, -1)
		for _, t := range tmp {
			t = strings.Trim(t, "\n")
