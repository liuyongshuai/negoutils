...
// 设置worker id
func (sfg *IdGenerator) SetWorkerId(w int64) *IdGenerator {
	sfg.lock.Lock()
...
// 设置时间戳占的位数
func (sfg *IdGenerator) SetTimeBitSize(n uint8) *IdGenerator {
	sfg.lock.Lock()
	defer sfg.lock.Unlock()
	sfg.isHaveInit = false
	sfg.timeBitSize = n
...
// 设置worker id占的位数
func (sfg *IdGenerator) SetWorkerIdBitSize(n uint8) *IdGenerator {
	sfg.lock.Lock()
	defer sfg.lock.Unlock()
	sfg.isHaveInit = false
...
// 设置序号占的位数
func (sfg *IdGenerator) SetSequenceBitSize(n uint8) *IdGenerator {
	sfg.lock.Lock()
	defer sfg.lock.Unlock()
	sfg.isHaveInit = false
...
// 初始化操作
func (sfg *IdGenerator) Init() (*IdGenerator, error) {
	sfg.lock.Lock()
	defer sfg.lock.Unlock()
...
// 解析生成的ID
func (sfg *IdGenerator) Parse(id int64) (int64, int64, int64, error) {
	//如果还没有初始化
	if !sfg.isHaveInit {
